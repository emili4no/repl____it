...
client.on("message", async message => {
  ...
  if (args[0] == `${prefix}start`) {
    if (
      ...
      if (!args[1] || !args[2] || !args[3])
        return message.channel.send(
          `**  Usage :** \`${prefix}start [الوقت] [عدد الفائزين] [الجايزة]\`   \n\**Example:** \`${prefix}start 10s 1 100k\``
        );
    ...
    if (
      ...
      if (!args[1])
        return message.channel.send(
          `**Usage:** **\`${prefix}gend [الايدي تبع رسالة الجيف اواي]\`**`
        );
      if (isNaN(args[1])) return message.channel.send(`Thats not a message ID`);
      message.channel
        .fetchMessage(args[1])
        .then(async m => {
          ...
          if (
            ...
            return message.channel.send(`هذا الجيف اواي انتهى بالفعل.`);
          if (m.reactions.size < 1)
            return message.channel.send(
              `I can't find reactions in this message.`
            );
...
●▬▬▬▬▬▬▬▬▬▬▬▬▬ஜ۩۞۩ஜ▬▬▬▬▬▬▬▬▬▬▬▬▬●
**
:yo_yo: | قائمة الجيف اواي
----------------------------
:white_flower: | __$start__ لبداء قيف اوي
----------------------------
developer : iObSidian
#7974
